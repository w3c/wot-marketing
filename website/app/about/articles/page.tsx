      ...
      <PageSection title="Articles and Press Releases">
        <Stack gap={2}>
          <Alert sx={{ display: 'block' }} variant="outlined">
            Note that the W3C does not guarantee the validity of the publications. When searching for other
            publications, include the &quot;W3C&quot; keyword to find content related to our standardization work. See
            also <Link href="https://en.wikipedia.org/wiki/Web_of_Things">Wikipedia</Link>.
          </Alert>
          <StyledTable
            header={[
      ...
      <PageSection title="Research Papers">
        <Stack gap={2}>
          ...
          <StyledTable
            ...
            rows={RESEARCH_PAPERS.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(
              (paper) => ({
                cells: [
                  paper.title,
                  paper.authors.join(', '),
