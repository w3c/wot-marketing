...
export async function generateMetadata(props: { params: Promise<{ lang: string; slug: string }> }): Promise<Metadata> {
  ...
  return pageMetadata(locale, `/participate/working-group/task-forces/${slug}`, {
    title: pageData.title,
    description: typeof pageData.description === 'string' ? pageData.description : undefined,
  });
}

export default async function TaskForceDynamicPage(props: { params: Promise<{ lang: string; slug: string }> }) {
