...
async function getGitHubData(url: URL): Promise<RepoData> {
  ...
  const pathParts = pathname.split('/').filter((part) => part.length > 0 && part !== 'tree' && part !== 'master');
...
async function getGitLabData(url: URL): Promise<RepoData> {
  ...
  try {
    ...
    const rootData = await safeFetch(`https://${host}/api/v4/projects/${encodedDir}?license=true`);
    const languages = await safeFetch(`https://${host}/api/v4/projects/${encodedDir}/languages`);
    if (!rootData.description) {
      ...
      rootData.description = extractToolMetadata(atob(readme.content)).description;
    }
...
function cleanLine(text: string): string {
  ...
  cleaned = cleaned.replace(new RegExp('\\[\\s*!\\[[^\\]]*\\]\\([^)]*\\)\\s*\\]\\([^)]*\\)', 'g'), '');
